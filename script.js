...
function addText(text, letter, number) {
  const output = document.getElementById('output');

  // Subscript letter and number (like E₁)
  const subscript = `<sub>${letter}${number}</sub>`;

  // Add sentence with subscript and a line gap
  output.innerHTML += (output.innerHTML.trim() ? '<br><br>' : '') + `${text} ${subscript}`;
}
